...
def fetch_announced_prefixes(asn):
    ...
    sys.stdout.write(
        f"{WARNING_ORANGE}  [>] Fetching prefixes for ASN {asn}... {RESET}"
    )
    sys.stdout.flush()
...
def save_to_txt(prefixes, filename):
    ...
    try:
        ...
        print(
            f"{BOLD}{SUCCESS_GREEN} │ [✓ SUCCESS] {len(prefixes)} unique ranges saved to {PROMPT_MAGENTA}'{filename}'{SUCCESS_GREEN}! │{RESET}"
        )
        print(
...
def main():
    ...
    while True:
        ...
        print(
            f"{BOLD}{DEEP_PURPLE} ║ {RESET}{BOLD}{ORANGE}[{RESET}{BOLD}{VIBRANT_PURPLE}1{RESET}{BOLD}{ORANGE}]{RESET} {VIBRANT_PURPLE}Single ASN Scan                             {RESET}{BOLD}{DEEP_PURPLE}║{RESET}"
        )
        print(
            f"{BOLD}{DEEP_PURPLE} ║                                                 ║{RESET}"
        )
        print(
            f"{BOLD}{DEEP_PURPLE} ║ {RESET}{BOLD}{ORANGE}[{RESET}{BOLD}{VIBRANT_PURPLE}2{RESET}{BOLD}{ORANGE}]{RESET} {VIBRANT_PURPLE}Multi-line ASN Input                        {RESET}{BOLD}{DEEP_PURPLE}║{RESET}"
        )
        print(
            f"{BOLD}{DEEP_PURPLE} ║                                                 ║{RESET}"
        )
        print(
            f"{BOLD}{DEEP_PURPLE} ║ {RESET}{BOLD}{ORANGE}[{RESET}{BOLD}{VIBRANT_PURPLE}3{RESET}{BOLD}{ORANGE}]{RESET} {VIBRANT_PURPLE}Scan ASNs From File                         {RESET}{BOLD}{DEEP_PURPLE}║{RESET}"
        )
        print(
            f"{BOLD}{DEEP_PURPLE} ║                                                 ║{RESET}"
        )
        ...
        if user_choice == "1":
            asn_input = input(
                f"{BRIGHT_WHITE}[{RESET}{PROMPT_MAGENTA}~{RESET}{BRIGHT_WHITE}]{RESET}{BOLD}{PROMPT_MAGENTA} Enter a single ASN: {RESET}"
            ).strip()
            if asn_input.isdigit():
                asn_list.append(asn_input)
            else:
                print(
                    f"{ALERT_RED}  [!] Invalid input: Please enter a valid number.{RESET}"
                )
                time.sleep(1.5)
                continue
        elif user_choice == "2":
            print(
                f"{BRIGHT_WHITE}[{RESET}{PROMPT_MAGENTA}~{RESET}{BRIGHT_WHITE}]{RESET}{BOLD}{PROMPT_MAGENTA} Enter ASNs, one per line (empty line to finish):{RESET}"
            )
            lines = []
            while True:
            ...
            filepath = input(
                f"{BRIGHT_WHITE}[{RESET}{PROMPT_MAGENTA}~{RESET}{BRIGHT_WHITE}]{RESET}{BOLD}{PROMPT_MAGENTA} Enter the path to your text file: {RESET}"
            ).strip()
            if os.path.exists(filepath):
                try:
                    ...
                    print(f"{ALERT_RED}  [!] Error reading file: {e}{RESET}")
                    time.sleep(1.5)
                    continue
            else:
                print(f"{ALERT_RED}  [!] File not found.{RESET}")
                time.sleep(1.5)
                continue
        else:
            print(f"{ALERT_RED}  [!] Invalid selection.{RESET}")
            time.sleep(1.5)
            continue
        ...
        if asn_list:
            ...
            print(
                f"{SUCCESS_GREEN}  [+] This scan added {newly_added_count} new unique prefixes.{RESET}"
            )
            print(
                f"{WARNING_ORANGE}  [>] Total unique prefixes in session: {len(session_prefixes)}{RESET}"
            )
    ...
    if session_prefixes:
        ...
        loading_animation(
            f"Cleaning all {len(session_prefixes)} collected prefixes ", 2
        )
